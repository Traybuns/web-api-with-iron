...
macro_rules! try_handler {
    ($e:expr) => {
        match $e {
            Ok(x) => x,
            Err(e) => return ok(Response::with((ststus::InternalServerError, e.description())))
        }
    };
    ($e:expr,$error:expr) => {
        match $e {
...
pub struct PostPostHandler {
 ...
 impl Handler for PostPostHandler{
    fn handle(&self,req:&mut Request) -> IronResult<Respose> {
        let mut payload = String::new();
        try_handler(req.body.read_to_string(&mut payload));

        let post = try_handler! (json::decode(&payload),status::BadReequest);
